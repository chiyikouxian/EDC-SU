...
`track_bridge` 已通过硬件验收，`TrackBridgeData_t` 的 7 字段合同已固化为正式 spec。硬件 A 也已确认底盘侧完成了 `track_bridge_get()` 消费适配（包括 PID 差速控制、丢线保护、找线、节点识别、刹车停车）。
...
- **新增可配置参数宏（默认值均来自 Hardware A 确认口径）：**
  - `CHASSIS_TRACK_ERROR_SCALE` (当前代码值 1)：将 bridge error `[-100, +100]` 映射到底盘 PID 误差尺度。
  - `CHASSIS_LOST_LINE_SHORT_TICKS` (当前代码值 18，TICK_MS=10 时 = 180ms) / `CHASSIS_LOST_LINE_LONG_TICKS` (当前代码值 180，TICK_MS=10 时 = 1800ms)：短时丢线原地找线，长时丢线刹车保护。
  - `CHASSIS_NODE_ACTIVE_COUNT_THRESHOLD` (当前代码值 6)：`active_count >= 6` 视为交叉/节点区域。
...
- **同步文档：** README 新增真实底盘接入说明、参数表、验收范围。
...
- 当前代码使用 `CHASSIS_TRACK_ERROR_SCALE = 1`，即 PID 直接消费 bridge error 的 `[-100, +100]` 尺度。
- `CHASSIS_TRACK_ERROR_SCALE` 是**底盘侧宏**，后续整车联调时可单独调整；主控侧长期合同仍固定为输出 `[-100, +100]`。
...
- `active_count >= CHASSIS_NODE_ACTIVE_COUNT_THRESHOLD`（当前代码值 5）视为交叉/节点区域。
