...
static uint8_t gray_read_out(void)
{
    ...
    for (sample = 0; sample < GRAY_SAMPLE_COUNT; sample++) {
        if (DL_GPIO_readPins(GRAY_SENSOR_PORT, GRAY_SENSOR_OUT_PIN) != 0) {
            high_count++;
        }
        if (sample + 1 < GRAY_SAMPLE_COUNT) {
            delay_cycles(CPUCLK_FREQ / 1000000 * GRAY_SAMPLE_INTERVAL_US);
        }
    }

    return (high_count * 2 >= GRAY_SAMPLE_COUNT) ? 1 : 0;
}
